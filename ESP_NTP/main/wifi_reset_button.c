...
{
	// 0.  Create the binary semaphore
	wifi_reset_semphore = xSemaphoreCreateBinary();

	///> todo use gpio_config insted of esp_rom_gpio_pad_select_gpio() for newer chips
	// 1. Configure the button and set the direction
	//esp_rom_gpio_pad_select_gpio(WIFI_RESET_BUTTON);	//esp idf 5.0
	esp_rom_gpio_pad_select_gpio(GPI_PIN);			//esp idf 4.4
	gpio_set_direction(GPI_PIN, GPIO_MODE_INPUT);
	gpio_set_pull_mode(GPI_PIN, GPIO_PULLUP_ONLY);

	// 2. Enable interrupt on the negative edge
	gpio_set_intr_type(GPI_PIN, GPIO_INTR_NEGEDGE);
	...
	// 3. Install gpio isr service
	gpio_install_isr_service(ESP_INTR_FLAG_DEFAULT);

	// 4. Attach the interrupt service routine
	gpio_isr_handler_add(GPI_PIN, wifi_reset_button_isr_handler, NULL);
}
