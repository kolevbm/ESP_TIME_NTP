		...
		if (xQueueReceive(http_server_monitor_queue_handle, &msg, portMAX_DELAY))
		{
			switch (msg.msgID)
			{
				...
				case HTTP_MSG_WIFI_USER_DISCONNECT:
					...
					g_wifi_connect_status = HTTP_WIFI_STATUS_DISCONECTED;
...
{
	set_led_work_state(LED_WORK_STATE_OTA_UPDATE);
	OTA_update = true;
	...
	if (!esp_timer_is_active (periodic_timer)&& !g_sta_connected)
	  {
...
static esp_err_t http_server_wifi_connect_json_handler(httpd_req_t *req)
{
	...
	app_nvs_clear_sta_creds();				// todo: remove this from here, add a web page button to clear NVS
...
static esp_err_t http_server_get_wifi_connect_info_json_handler(httpd_req_t *req)
{
	ESP_LOGI(TAG, "/wifiConnectInfo.json requested");
  	...
  	//uint8_t IP4ADDR_STRLEN_MAX = 16;    // could not resolve symbol
...
/**
 * localTime.json handler responds by sending local time
 * @param req HTTP request for which the uri needs to be handled.
 * @return ESP_OK
 */
...
/**
 * apSSID.json handler responds by sending ESP32 Access Point SSID
 * @param req HTTP request for which the uri needs to be handled.
 * @return ESP_OK
 */
